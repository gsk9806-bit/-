...
```json
{
  ...
  "caption": { "body": "캡션 본문", "tags": "#해시태그 #readandlive" }
}
```
...
- `caption` = 태도2 출력의 **블록2(캡션)**. `body`는 캡션 본문 문장, `tags`는 해시태그
  줄을 그대로 옮긴다. 카드 위쪽에 별도 칸으로 뜨고, **텍스트 복사**에도 함께 담긴다
...
- `key`는 회차마다 바꾼다. 브라우저 localStorage 키라서, 같으면 이전 회차 편집분이 되살아난다.
  캡션은 `key + "__caption"` 이라는 별도 키에 저장되므로 카드 저장분과 서로 간섭하지 않는다
